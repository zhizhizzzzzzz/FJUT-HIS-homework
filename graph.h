...
public:
    ...
    Graphlnk(const vector<tuple<T, T, E>>& edges); // 通过边信息列表构造函数
    ~Graphlnk(); // 析构函数
...
// 构造函数:通过边信息列表建立邻接表表示的图
template <class T, class E>
Graphlnk<T, E>::Graphlnk(const vector<tuple<T, T, E>>& edges) {
    // 使用传递的边信息列表计算顶点数
    set<T> vertices;
    for (const auto& edge : edges) {
        vertices.insert(get<0>(edge));
        vertices.insert(get<1>(edge));
    }
    ...
    for (const auto& edge : edges) {
        ...
        int j = getVertexPos(e1);
        if (j == -1) {
            insertVertex(e1);
            j = getVertexPos(e1);
        }

        int k = getVertexPos(e2);
        if (k == -1) {
            insertVertex(e2);
            k = getVertexPos(e2);
        }
